						...
						<form method="GET" action="{{ route('admin.dashboard') }}" class="mb-4">
							<div class="flex space-x-4">
								...
								<div>
									...
									<input type="date" id="fecha" name="fecha" value="{{ request('fecha', date('Y-m-d')) }}" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-violet-300 focus:outline-none focus:ring-violet-500 focus:border-violet-500 sm:text-sm rounded-md">
								</div>
						...
						<table class="min-w-full bg-white">
							...
							<tfoot class="bg-gray-100 text-gray-800">
								<tr>
									...
									<td class="text-left py-3 px-4 uppercase font-semibold">$ {{ $transacciones->sum('precio_servicio') }}</td>
									...
									<td class="text-left py-3 px-4 uppercase font-semibold">$ {{ $transacciones->sum('monto_empleado') }}</td>
									<td class="text-left py-3 px-4 uppercase font-semibold">$ {{ $totalDescuentos }}</td>
									<td class="text-left py-3 px-4 uppercase font-semibold">$ {{ $transacciones->sum('total_transaccion') }}</td>
									<td></td>
								</tr>
							</tfoot>
						</table>
